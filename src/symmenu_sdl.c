...
static symmenu_sdl_render_t *make_symmenu_sdl_render(symmenu_t *menu, SDL_Surface *surface) {
	symmenu_sdl_render_t *render = calloc(1, sizeof(symmenu_sdl_render_t));
	if (render == NULL) {
...
/* Use the preferences struct to initialize SDL render/cache state for symmenu. */
symmenu_sdl_render_t *symmenu_sdl_render(SDL_Surface *screen, pref_t *prefs, symmenu_t *menu) {
	...
	return make_symmenu_sdl_render(menu, menu_surface);
}
