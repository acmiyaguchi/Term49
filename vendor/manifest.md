...
SDL/libconfig/TouchControlOverlay are pinned source submodules plus checked-in
known-good BB10 prebuilts. Lua is plain C compiled directly by `qcc` (no Zig/nix
step, unlike libghostty-vt) and statically linked, so it adds no new
`prebuilt-bb10` artifact or `bar-descriptor.xml` asset.
